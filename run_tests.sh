...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testopts" = "" ] && [ "$testargs" = "" ]; then
      ...
      # provided.
      testargs="discover ./metaplugin/tests"
    fi
  ...
  OS_TEST_PATH=`echo $testargs|grep -o 'metaplugin\.tests[^[:space:]:]\+'|tr . /`
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='metaplugin/*' --omit='metaplugin/openstack/common/*' -d covhtml -i
  fi
